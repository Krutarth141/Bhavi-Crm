            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <div>
                    ...
                    <div style={{ fontSize: 12, color: '#6b7280', marginTop: 2 }}>
                        Last refreshed: {lastRefresh.toLocaleTimeString('en-IN')} | Auto-refresh every 2 min
                    </div>
                </div>
                <button onClick={refetch} disabled={loading} style={{ padding: '8px 14px', border: '1px solid #e5e7eb', borderRadius: 6, cursor: 'pointer', fontSize: 13, background: 'white', opacity: loading ? 0.6 : 1 }}>
                    🔄 Refresh
                </button>
            </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 16 }}>
                    ...
                    <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden' }}>
                        ...
                        {engineers.length === 0 ? (
                            ...
                            engineers.map(eng => {
                                const online_ = isOnline(eng.recorded_at);
                                const isSel = selected === eng.eng_name;
                                return (
                                    <div
                                        key={eng.eng_name}
                                        onClick={() => setSelected(isSel ? null : (eng.eng_name || null))}
                                        style={{ padding: '12px 16px', borderBottom: '1px solid #f3f4f6', cursor: 'pointer', background: isSel ? '#eff6ff' : 'white', borderLeft: isSel ? '3px solid #185FA5' : '3px solid transparent' }}
                                    >
                                        ...
                                        <div style={{ fontSize: 12, color: '#374151', marginTop: 2 }}>
                                            {eng.event_type ? `📌 ${eng.event_type}` : ''}
                                            {eng.ticket_id ? ` | Ticket: ${eng.ticket_id}` : ''}
                                        </div>
                                        <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>{timeAgo(eng.recorded_at)}</div>
                                    </div>
                                );
                            })
                        )}
                        ...
                        <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 8, overflow: 'hidden' }}>
                            <div style={{ padding: '12px 16px', borderBottom: '1px solid #e5e7eb' }}>
                                ...
                                <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                                    <span style={{ padding: '2px 8px', borderRadius: 10, fontSize: 11, fontWeight: 600, background: isOnline(sel.recorded_at) ? '#d1fae5' : '#f3f4f6', color: isOnline(sel.recorded_at) ? '#065f46' : '#6b7280' }}>
                                        {isOnline(sel.recorded_at) ? '🟢 Online' : '⚫ Offline'}
                                    </span>
                                    <span style={{ fontSize: 12, color: '#6b7280', padding: '2px 0' }}>{timeAgo(sel.recorded_at)}</span>
                                </div>
                            </div>
                            <div style={{ padding: 16 }}>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                                    {[
                                        { label: 'Coordinates', value: sel.lat && sel.lng ? `${sel.lat.toFixed(5)}, ${sel.lng.toFixed(5)}` : '—' },
                                        { label: 'Accuracy', value: sel.accuracy ? `±${sel.accuracy}m` : '—' },
                                        { label: 'Event Type', value: sel.event_type || '—' },
                                        { label: 'Session Date', value: sel.session_date || '—' },
                                        { label: 'Ticket', value: sel.ticket_id || '—' },
                                        { label: 'Last Updated', value: sel.recorded_at ? new Date(sel.recorded_at).toLocaleString('en-IN') : '—' },
                                    ].map(item => (
                                ...
                                {sel.lat && sel.lng ? (
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 16px', background: '#185FA5', color: '#fff', borderRadius: 6, textDecoration: 'none', fontSize: 13, fontWeight: 600 }}
                                    >
                                ...
                                <div style={{ maxHeight: 200, overflowY: 'auto' }}>
                                    {locations.filter(l => l.eng_name === sel.eng_name).slice(0, 10).map(l => (
                                        <div key={l.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #f3f4f6', fontSize: 12 }}>
                                            ...
                                            <span style={{ color: '#9ca3af', marginLeft: 8, whiteSpace: 'nowrap' }}>{timeAgo(l.recorded_at)}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
